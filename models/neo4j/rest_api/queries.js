  ...
  graph: function() {
    var queries = {
      ...
      relationships:{
        ...
        create_REVIEWED: 'MATCH (p:Person), (m:Movie) WHERE id(p)={source} AND id(m)={target} CREATE (p)-[r:REVIEWED {summary: {property}, rating: {rating} }]->(m) RETURN p.name, type(r), id(r), m.title, r.summary',
        ...
        read: 'MATCH (m)-[r]->(n) WHERE id(r)={id} RETURN coalesce(m.title, m.name), type(r), coalesce(n.title, n.name), coalesce(r.roles, r.summary), r.rating',
        ...
        getUpdate: 'MATCH (m)-[r]->(n) WHERE id(r)={id} RETURN coalesce(m.title, m.name), type(r), coalesce(n.title, n.name), coalesce(r.roles, r.summary), r.rating',
        ...
        update_REVIEWED: 'MATCH (m)-[r]->(n) WHERE id(r)={id} SET r.summary={property}, r.rating={rating} RETURN coalesce(m.title, m.name), type(r), coalesce(n.title, n.name), coalesce(r.roles, r.summary), r.rating',
