        ...
        that.krypto.hashUser(user, function(err, userKey) {
          ...
          redisClient.exists(userKey.toString('utf8'), function(error, exists) {
            ...
            if (parseInt(exists.toString('utf-8')) === 1 ) {
              return callback(null, {id:userKey, username: user});
            } else {
      ...
      get: function(user, callback)  {
        that.krypto.hashUser(that.krypto.decryptUserName(user, that.usersConfig.secret), function(err, userKey) {
          ...
          redisClient.hgetall(userKey.toString('utf8'), function(err, userObject) {
            ...
            if (userObject) {
              userObject.user = that.krypto.decryptUserName(user, that.usersConfig.secret);
        ...
        that.krypto.hashUser(user, function (err, userKey) {
          ...
          that.krypto.hashPassword(userObject.password, function (err, passwordValue) {  
            ...
            redisClient.exists(userKey.toString('utf8'), function(error, exists) {
              ...
              if (parseInt(exists.toString('utf-8')) === 1 ) {
                return callback(null, parseInt(exists.toString('utf-8'))); // null???
              } else {
                redisClient.hmset(userKey.toString('utf8'), {
                  'user': that.krypto.encryptUserName(user, that.usersConfig.secret),
                  // 'authType': 'localAuth',
                  ...
                  'password': passwordValue,
                    ...
                    // if (reply === ?) {} else {} ???
                    return callback(null, exists, userKey, passwordValue); // parseInt(exists.toString('utf-8'))???
                });
              }
            });
          });
      ...
      authenticate: function (user, password, callback) {
        ...
        that.krypto.hashUser(user, function (err, userKey) {
          ...
          redisClient.hgetall(userKey.toString('utf8'), function(err, userObject) {
            ...
            // switch case 'authType': 'localAuth', oauth, etc
